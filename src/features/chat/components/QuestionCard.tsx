...
/**
 * AskUserQuestion card: the CLI parked the question on the control protocol
 * (`can_use_tool`) and the turn only continues once it is answered or
 * skipped. Answers are the chosen option labels — that is the contract the
 * CLI's question reader matches against (typed free text would arrive as a
 * "typed answer", which this card does not offer yet).
 */
export function QuestionCard({ message }: { message: Message }) {
  ...
  const answer = () => {
    if (key && complete) void respondToQuestion(key, message.seq, picked);
  };
  const skip = () => {
                  ...
                  {q.options.map((opt) => {
                    ...
                    return (
                      <button
                        ...
                        className={`flex cursor-pointer flex-col items-start rounded-md border px-2.5 py-1.5 text-left transition-colors ${
                          selected
