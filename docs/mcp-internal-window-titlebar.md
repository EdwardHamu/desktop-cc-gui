...
## 后续修复：React Hook 测试问题已解决

此前 5 项组件失败已定位为 Windows 盘符大小写导致 React 双实例，与生产 Hook 实现无关。统一 Vitest root/alias/cwd 后，标题栏 6 项与窗口/关闭边界 14 项全部通过，未更改这些测试断言或 mock 掉 Hook。另含聊天组件等在内共 37 项回归通过。完整证据见 docs/mcp-react-hook-test-runtime.md；原生验收仍未执行。
